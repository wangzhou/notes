...
配置到SMMU上即可。注意，SMMU spec支持对STE或者CD配置partid，外设的流量可以通过CD
进一步细分。
...
但可以通过不同reqPARTID区分监控上下文。注意，这里是反过来配置，使用intPARTID做控制，
使用reqPARTID做监控。

举个例子，memory MSC(有narrow，32 intPARTID，256 reqPARTID)，PMG有8个值。无narrow时，
监控靠(PARTID, PMG)，最多256×8 = 2048个监控端点。有narrow时，监控靠(reqPARTID, PMG)，
最多256×8 = 2048个监控端点。

如果，因为其他的原因，控制组只能是32，那么不用narrow时，最大监控组数为32x8(partid x pmg) = 256。
在有narrow的情况下，可以用narrow后的intPARTID作为控制组(32)，不narrow的reqPARTID+pmg
作为监控组，这样监控组就可以有256x8=2048个。
  ...
  +-> acpi_mpam_parse_resources     <-- 创建mpam_ris
    +-> mpam_ris_create 
      +-> mpam_class_get            <-- 如果还没有，就创建一个
           ...
           * (schemata中的一行)，是该域的最小配置单元。(todo: emm, 如果一个NUMA里有多个内存相关的MSC会怎么样?)
           */
      +-> mpam_component_get
  +-> mpam_register_cpuhp_callbacks(&mpam_discovery_cpu_online)

mpam_discovery_cpu_online           <-- 如上probe里注册，cpu online时执行。(todo: 为什么这么设计？)
