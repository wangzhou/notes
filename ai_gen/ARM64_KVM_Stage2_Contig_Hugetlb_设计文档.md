...
如上walk中[addr, addr+size]可能是跨越多个contig block的，对于不对齐的addr，可以
把不对齐addr对应的contig block先删除contig bit，然后做对应处理，对于对齐的contig
block，可以直接在一次walk的时候直接处理。

想到另外一个问题，在热迁移的时候，如果contig block还在，整个contig block改成写保护，
stage2 page fault的时候时候，处理粒度会不会按contig block。如果是，这样是不对的，
因为热迁移还是按基础页粒度的。(新问题)
