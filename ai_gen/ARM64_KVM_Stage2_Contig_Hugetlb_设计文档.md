...
注意，contig bit hugetlb对应的VMA，有可能被host侧的mprotect/madvice等虚拟地址管理
工具在改变虚拟地址属性的时候拆掉，但是不会改变VMA的vma_pagesize，比如，4K-64K的
contig bit hugetlb，vma_pagesize还是64KB。

这个约束意味着，如上map/relax_perms路径(stage2 fault进来)，vma_pagesize都是hugetlb
大小，这样软件代码处理会都走到CONT_PTE_SIZE的路径上。

这个意味着，并发场景下，目前看，只有迁移过程中，会因为force_pte而出现单页fault的
情况。如下可以看到，迁移时会有contig pte和单pte的并发问题，这个问题还没有解决。
...
这一节，不关注其他条件。单纯说明contig pte和单pte的问题。
...
**结论**：dirty logging SRCU 窗口是可确认的触发路径，可致丢失更新。
          (todo: 增加对应的WARN检测这个race是否可以跑出来)
...
**结论**：
- 场景 1a（map contig vs map 单页）在 dirty logging 关闭时的 SRCU 窗口中可触发，双向丢失更新，需要修复。
- 场景 2（contig BBM vs mkyoung/relax_perms）可触发，后果可自愈（AF/权限丢失触发额外 fault 即可恢复），不影响正确性。
