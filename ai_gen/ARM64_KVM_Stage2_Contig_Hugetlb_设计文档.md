...
`force_pte` 差异），即可触发 contig 升级与单页 BBM 在 PTE[k] 上的竞态（见 (三)）。
关键是 mprotect/madvise 制造了 PTE[0] = valid 非 CONT 的**前置条件**，而
`force_pte` 分歧（不管来源是什么）利用了这个条件。

**（三）场景 1：contig BBM vs map 单页 —— 核心竞态**
...
vma_pagesize 分歧机制，详见 (二) 节。mprotect/madvise 间接触发 MMU notifier
（partial unmap/wrprotect）unfold contig block，制造 PTE[0] = valid 非 CONT
的前置条件，见 (二) 节分析。contig 之前无同类问题——force_pte 差异导致的是跨层级
