...
(理解如下的分析，需要先理解KVM stage2 walk的基本逻辑，具体可以参考[这里](https://wangzhou.github.io/arm64-KVM-Stage2页表映射流程分析/))。
...
目前，只修改一个PTE的BBM逻辑是：锁PTE -> clear PTE -> tlbi -> 更新PTE -> 解锁PTE。
具体实现的时候，前两个步骤合并成对PTE原子的更新一个域段，最后两个步骤同样的逻辑。

所以，扩展到一个改一堆PTE，也要遵守类似的逻辑，只不过这里变成了一堆PTE。我们采用
这一堆PTE的第一个PTE作为锁PTE要争抢的变量。整个BBM流程如下:
```
...
|----------|------|--------|--------|------|--------|---------|------------|-------------|
| `stage2_map` | 缺页映射 | fault (`user_mem_abort`) | map_walker | `read_lock` / pKVM `write_lock`⁷ | ✅ | cmpxchg | ✅ / ❌⁷ | contig_leaf / PTE[0]检查 |
| `stage2_set_owner` | pKVM owner 标记 | pKVM EL2 | map_walker | exclusive⁸ | ❌ | WRITE_ONCE | ❌ | 不冲突 |
| `stage2_relax_perms` | 放宽权限 | fault（写故障） | attr_walker | `read_lock` / pKVM `write_lock`⁷ | ✅ | cmpxchg | ✅ / ❌⁷ | contig_leaf / PTE[0]检查 |
| `stage2_mkyoung` | 设 AF | fault（AF 故障） | attr_walker | `read_lock` / pKVM `write_lock`⁷ | ✅ | cmpxchg | ✅ / ❌⁷ | contig_leaf / PTE[0]检查 |
| `stage2_wrprotect` | 写保护 | 系统调用（dirty log） | attr_walker | `write_lock` | ❌ | WRITE_ONCE | ❌ | contig_leaf / PTE[0]检查¹ |
| `stage2_test_clear_young` | 清 AF + 老化 | MMU notifier (ksm/khugepaged/madvise/kswapd⁶) | age_walker | `write_lock` / lockless⁴ | ❌ | WRITE_ONCE | ❌/✅⁴ | lockless 时编译互斥禁用 contig⁵ |
| `stage2_unmap` | 取消映射 | ioctl / pKVM — 系统调用、VM teardown<br>MMU notifier — mprotect、madvise、munmap、THP collapse/split² | unmap_walker | `write_lock` | ❌ | WRITE_ONCE | ❌ | contig unmap |
| `stage2_destroy_range` | 销毁范围 | VM teardown / MMU notifier release³ | destroy_walker | exclusive | ❌ | WRITE_ONCE | ❌ | 不涉及（整表销毁） |
...
>
> ² MMU notifier `invalidate_range_start` 回调，由 host 侧 mprotect、madvise
> （MADV_DONTNEED/MADV_FREE）、munmap、THP collapse/split、KSM 等触发，通过
> `kvm_mmu_notifier_invalidate_range_start` → `kvm_handle_hva_range`（持
> `KVM_MMU_LOCK`=`write_lock`）→ `kvm_mmu_unmap_gfn_range` →
> `__unmap_stage2_range` → `kvm_pgtable_stage2_unmap`。持 `write_lock` 排他，
> 与 fault 路径（read_lock）不并发，与 `stage2_test_clear_young`、系统调用
> 触发的 unmap/wrprotect 等 `write_lock` 持有者也不并发。
>
> ³ `mmu_notifier_release` 在 VM 销毁时触发，无并发 vCPU。整表 `stage2_destroy_range`
> 逐级回收页表页，不参与正常并发模型。
>
...
>
> ⁶ test_clear_young 的 MMU notifier 触发源：ksm（页共享扫描）、khugepaged
> （THP collapse 老化检查）、madvise（MADV_COLD / MADV_PAGEOUT）、kswapd
> （内存回收 page_referenced）。通过 `clear_flush_young` / `clear_young` /
> `test_young` 回调进入 `kvm_age_gfn` / `kvm_test_age_gfn` →
> `kvm_pgtable_stage2_test_clear_young`。
>
> ⁷ `kvm_fault_lock` 在 pKVM (`is_protected_kvm_enabled()`) 下取 `write_lock`
> 而非 `read_lock`。pKVM 中所有 fault 路径（map/relax_perms/mkyoung）都是
> `write_lock` 排他，**彼此不并发**，PTE[0] 锁的 cmpxchg 串行化在 pKVM 中
> 是冗余的（但无害）。非 pKVM 下仍为 `read_lock` 共享。
>
> ⁸ pKVM EL2（`__pkvm_*` / `host_stage2_*`）直接操作 `host_mmu.pgt` 或
> `vm->pgt`，运行于 EL2 不可抢占上下文，不持 host 的 `mmu_lock`。"exclusive"
> 指 EL2 单线程执行保证的无并发语义。`stage2_unmap` 在 EL2 侧（如
> `host_stage2_unmap_dev_all`）同理——不走 `__unmap_stage2_range` 的
> `lockdep_assert_held_write` 路径，而是 EL2 直接调用 `kvm_pgtable_stage2_unmap`。
> 表中 unmap 行里的 "pKVM" 指 EL1 pKVM host 路径（持 `write_lock`），EL2
> unmap 与 `stage2_set_owner` 同属 exclusive 域。

并发视图（非 pKVM，`kvm_fault_lock` = `read_lock`）：
```
...
write_lock(&kvm->mmu_lock)                               // 系统调用 / MMU notifier，排他
  ├─ wrprotect:         非SHARED, WRITE_ONCE → 无并发，PTE[0] 检查代码无害
  ├─ test_clear_young:  非SHARED, WRITE_ONCE → 无并发（非lockless）；lockless 时编译互斥禁用 contig
  ├─ unmap (ioctl):     非SHARED, WRITE_ONCE → 无并发，contig unmap
  └─ unmap (MMU notifier): 非SHARED, WRITE_ONCE → 无并发，contig unmap
```

pKVM 下 `kvm_fault_lock` = `write_lock`，fault 路径也排他——map/relax_perms/mkyoung 之间无并发，
PTE[0] cmpxchg 串行化冗余但无害。EL2 侧 set_owner / unmap 走 exclusive 域，不与 EL1 路径竞争
`mmu_lock`。实际需要 PTE[0] 互斥保护的场景是非 pKVM 下 fault 路径间的并发。
...
**（一）并发矩阵：T1 为 contig BBM，T2 为各路径**
...
**（二）场景 1：contig BBM vs map 单页 —— 核心竞态**

T1 走 contig BBM（`stage2_map_contig_leaf`），T2 走单页 BBM
（`stage2_map_walker_try_leaf`），同属 `user_mem_abort` map 路径，均持 `read_lock`。
...
单页路径见 PTE[0]=valid 非 CONT 时不锁 PTE[0]；T1 在 PTE[0] 上的 cmpxchg 可以
成功（PTE[0] 当前确是那个 valid 值）。两路在 PTE[k] 上无锁碰撞。

**vma_pagesize 分歧与 SRCU 窗口**。T1 和 T2 对同一 contig-aligned 区域得出不同的
`vma_pagesize`——T1=CONT_PTE_SIZE（prot 带 PROT_CONT），T2=PAGE_SIZE（走单页）。
`vma_pagesize` 由 `force_pte ? PAGE_SHIFT : get_vma_page_shift(...)` 决定，
`force_pte = memslot_is_logging(memslot)`。

vCPU fault handler 通过 `gfn_to_memslot()` 读 memslot flags 不持 `slots_lock`。
ioctl 修改 memslot 用 SRCU 保护：准备新副本 → `rcu_assign_pointer` 原子切换 →
`synchronize_srcu_expedited` 等老 reader 退出。SRCU 保证单个 reader 数据内部一致，
但**不保证所有 reader 同时看到新版本**——这为 `force_pte` 分歧提供了窗口。

具体触发：QEMU 关闭 dirty logging（`KVM_SET_USER_MEMORY_REGION2` 清
...
```
关键：disable 分支**不拿 mmu_lock**，老 reader 的 `mmu_invalidate_retry()` 返回
false，携 `force_pte=true` 继续。对比 logging 开启：enable 分支通过
...
**mprotect/madvise 的间接作用**：mprotect/madvise/THP 不改变 hugetlb VMA 类型，
`get_vma_page_shift` 返回值一致。但 mprotect/madvise 间接触发 MMU notifier——
KVM 回调中的 partial unmap/wrprotect 会 unfold contig block，把 PTE[0] 从
valid+CONT 变为 valid 非 CONT，制造上述竞态的**前置条件**。contig 之前无同类问题
——原来 force_pte 差异导致跨层级分歧（PTE 级 vs PMD 级），不在同一 entry 竞争。

**结论**：dirty logging SRCU 窗口是可确认的触发路径，可致丢失更新。测试表明正常起
虚机（无 logging 开关）时 WARN 概率触发，说明存在额外触发路径，需继续排查。修复见
5.4.3.2。

**（三）场景 2：contig BBM vs attr mkyoung / relax_perms**
...
**（四）场景 3：contig BBM vs test_clear_young（lockless）——已通过编译时互斥解决**

lockless 路径下 age_walker 会直接修改单个 PTE 的 AF 位，无视 CONT 约束，破坏
contig block 一致性。适配 age_walker 需完整 BBM 流程（锁 PTE[0]、清 N 个 PTE、
TLBI、重写），与 lockless 的无锁语义冲突。当前方案：编译时互斥——

在 `user_mem_abort` 的 `CONT_PTE_SHIFT` 分支中增加
`IS_ENABLED(CONFIG_KVM_MMU_LOCKLESS_AGING)` 检查，lockless 配置下直接退化到
PAGE_SHIFT，不设 `KVM_PGTABLE_PROT_CONT`。无运行时开销。

**（五）场景 4：contig BBM vs contig BBM（无问题）**
...
|------|----|---------|---------|-------|------|
| 1a | contig BBM | map 单页（logging 关闭 SRCU 窗口） | **是（低概率）** | cmpxchg 竞态 | 双向丢失更新 |
| 1b | contig BBM | map 单页（mprotect/madvise 间接触发 unfold + force_pte 分歧） | **待确认** | — | 不改 VMA 类型，但制造 PTE[0]=valid 非 CONT 前置条件，见 (二) |
...
##### 5.4.3.3 残留窗口：PTE[0]=valid 非 CONT 时的 PTE[k] 竞态
