...
**为什么 attr_walker 的 PTE[0] 检查不加 `walk_shared` 门**：

attr_walker 服务于三条路径：relax_perms（read_lock, SHARED），mkyoung（read_lock, SHARED），
wrprotect（write_lock, !SHARED）。前两者与 contig BBM 存在真实并发，需要 PTE[0] 检查。

如果加 `walk_shared` 门：relax_perms 和 mkyoung 已有 SHARED ⇒ 会进入检查（应该的）；
wrprotect 无 SHARED ⇒ 不会进入。但 wrprotect 持 write_lock 排他，没有并发，不进入也无害。
所以加不加门结果一样。

不加门的理由：1) 代码更简洁；2) 未来如果 wrprotect 改为 read_lock 或加 SHARED，代码
已就位；3) write_lock 下多做一次 PTE[0] READ_ONCE，O(1) 开销不可测。

map path 不需要放宽：非 SHARED map 只有 pKVM set_owner，跑 exclusive 锁。
...
##### 5.4.3.2 修复实现：PTE[0] 作为 block 级互斥点
